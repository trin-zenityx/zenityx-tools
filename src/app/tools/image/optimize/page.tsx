...
export default function OptimizePage() {
  ...
  const handleDownloadAll = async () => {
    ...
    downloadBlob(blob, `zenityx-optimize-${Date.now()}.zip`);
  };
  ...
  return (
    <PageShell
      ...
      breadcrumbs={[
        ...
        { label: "บีบอัดภาพ", href: "/tools/image/optimize" }
      ]}
    >
      <div className="grid">
        <div className="form-panel">
          ...
          <select
            ...
            className="input"
            value={format}
            onChange={(event) => setFormat(event.target.value as OptimizeFormat)}
          >
            <option>JPG</option>
            <option>WEBP</option>
          </select>

          <label htmlFor="quality">ระดับคุณภาพ ({quality}%)</label>
          <input
            id="quality"
            className="input"
            type="range"
          ...
          />
          <span className="helper">เลื่อนไปทางขวาเพื่อคุณภาพสูงขึ้น</span>

          <label htmlFor="target">ขนาดเป้าหมาย (KB)</label>
          <input
            id="target"
            className="input"
            type="number"
